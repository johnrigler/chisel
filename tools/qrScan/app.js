...
function onInputOpReturnHex() {
  ...
  window.loadCurrencyWif = function loadCurrencyWif(currencyKey, wif) {
    if (!currencyKey) {
    ...
    setSenderWifValue(wif);
    setStatusMessage("Scanned WIF loaded. Review the currency and fee before sending.", false);
  };

  window.loadCurrencyWifAndSend = function loadCurrencyWifAndSend(currencyKey, wif) {
    window.loadCurrencyWif(currencyKey, wif);
    elems.sendButton.click();
  };
  ...
  window.addEventListener("message", function onMessage(event) {
    ...
    if (!data || data.type !== "chisel.loadWif") {
      return;
    }

    window.loadCurrencyWif(data.currency || DEFAULT_CURRENCY_KEY, data.wif);
  });


function readPendingQrScanPayload() {
  const params = new URLSearchParams(window.location.search);
  const payload = {
    currency: params.get("currency") || params.get("coin") || "",
    wif: params.get("wif") || params.get("senderWif") || "",
    autosend: params.get("autosend") === "1" || params.get("send") === "1"
  };

  if (!payload.wif && params.get("loadScannedWif") === "1") {
    try {
      const stored = window.sessionStorage.getItem("chisel.pendingQrScan") ||
        window.localStorage.getItem("chisel.pendingQrScan");

      if (stored) {
        const parsed = JSON.parse(stored);
        payload.currency = parsed.currency || payload.currency;
        payload.wif = parsed.wif || "";
        payload.autosend = parsed.autosend === true || payload.autosend;
      }
    } catch (error) {
      console.warn("Could not read chisel.pendingQrScan", error);
    }
  }
  ...
  if (payload.wif) {
    try {
      ...
      console.warn("Could not clear chisel.pendingQrScan", error);
    }
  }
...
function applyPendingQrScanPayload() {
  const payload = readPendingQrScanPayload();
  ...
  if (payload.currency) {
    try {
      ...
      console.warn("Could not apply scanned currency", error);
    }
  }
