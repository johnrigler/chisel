...
//  const DEFAULT_RPC_URL = "https://rigler.org:8769/";
  ...
  const NAMESPACE = "ravencoin";

  //
  // Plugin
  //
  function installChiselRavencoin(CHISEL) {
    if (!CHISEL) {
      throw new Error("CHISEL is required.");
    }

    if (!CHISEL.base58CheckDecode) {
      throw new Error("installChiselSigner must be called before installChiselRavencoin.");
    }
  ...
  window.installChiselRavencoin = installChiselRavencoin;
})();
